...
function Invoke-PackageInstall {
    ...
    try {
        switch ($PackageManager) {
            'scoop' {
                if ($GlobalInstall) {
                    gsudo scoop install -g $PackageName 2>&1 | Out-Null
                }
                else {
...
function Install-Packages {
    ...
    $header = if ($Label) { "$Label ($($Packages.Count) packages)" } else { "Installing $($Packages.Count) package(s) using $PackageManager" }
